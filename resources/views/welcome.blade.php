            ...
            <div class="flex justify-center mt-6">
                @if ($contatos->hasPages())
                    <nav role="navigation" class="inline-flex space-x-2">
                        ...
                        @else
                            <a href="{{ $contatos->previousPageUrl() }}" class="px-4 py-2 text-white rounded-lg bg-themeColor hover:bg-themeColorLight">
                                Anterior
                            </a>
                        @endif
                        @foreach ($contatos->getUrlRange(1, $contatos->lastPage()) as $page => $url)
                            ...
                            @else
                                <a href="{{ $url }}" class="px-4 py-2 bg-gray-200 rounded-lg text-themeColor hover:bg-gray-300">
                                    {{ $page }}
                                </a>
                            @endif
                        @endforeach
                        @if ($contatos->hasMorePages())
                            <a href="{{ $contatos->nextPageUrl() }}" class="px-4 py-2 text-white rounded-lg bg-themeColor hover:bg-themeColorLight">
                                Próximo
                            </a>
                        @else
